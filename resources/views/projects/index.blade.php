...
<div class="flex items-center justify-between mb-6">
    ...
    <div class="flex items-center gap-2">
        <a href="{{ route('exports.projects') }}" class="inline-flex items-center gap-2 bg-white hover:bg-emerald-50 text-emerald-700 text-sm font-semibold px-4 py-2.5 rounded-lg shadow-sm border border-emerald-200 transition" title="Download all projects as Excel">
...
{{-- Create modal extracted to a partial so the dashboard quick-action can
     reuse the same HTML — single source of truth. --}}
...
<div id="editModal" class="hidden fixed inset-0 z-50 flex items-center justify-center modal-overlay" data-modal-id="editModal">
    <div class="bg-white rounded-xl shadow-2xl w-full max-w-2xl mx-4">
        ...
        <form id="editForm" class="p-6 space-y-4">
            ...
            {{-- 2026-07-16 (Brenda): job site address — shows on the PO Ship To. --}}
...
function editProject(id){
    $.get(BASE_URL+'/projects/'+id+'/edit', function(d){
        ...
        f.querySelector('[name="state"]').value=p.state||'';
        f.querySelector('[name="zip"]').value=p.zip||'';

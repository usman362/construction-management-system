...
<div class="flex items-center justify-between mb-6">
    ...
    <div class="flex items-center gap-2">
        <a href="{{ route('clients.import.template') }}" class="inline-flex items-center gap-2 bg-white hover:bg-gray-50 text-gray-700 text-sm font-semibold px-4 py-2.5 rounded-lg shadow-sm border border-gray-200 transition">
...
<div id="importModal" class="hidden fixed inset-0 z-50 flex items-center justify-center modal-overlay" data-modal-id="importModal">
    <div class="bg-white rounded-xl shadow-2xl w-full max-w-lg mx-4">
        <div class="flex items-center justify-between px-6 py-4 border-b border-gray-100">
            <h3 class="text-lg font-bold text-gray-900">Import Clients from CSV</h3>
        ...
        <form method="POST" action="{{ route('clients.import') }}" enctype="multipart/form-data" class="p-6 space-y-4">
            @csrf
            <div class="bg-blue-50 border border-blue-200 rounded-lg p-4 text-sm text-blue-900">
                ...
                <ol class="list-decimal list-inside space-y-0.5 text-xs">
                    ...
                    <li>Existing clients are matched first by <code>vendor_code</code>, then by exact <code>name</code> — matches are updated.</li>
                </ol>
            </div>
            <div>
    ...
    <div class="bg-white rounded-xl shadow-2xl w-full max-w-lg mx-4">
        ...
        <form id="createForm" class="p-6 space-y-4">
            ...
            <div class="grid grid-cols-3 gap-4">
                ...
                <div><label class="block text-sm font-medium text-gray-700 mb-1">ZIP Code</label><input type="text" name="zip" class="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:ring-2 focus:ring-blue-500 focus:border-blue-500 outline-none"></div>
            </div>
        </form>
        <div class="flex items-center justify-end gap-3 px-6 py-4 bg-gray-50 border-t border-gray-100">
            ...
            <button onclick="submitForm('createForm','{{ route("clients.store") }}','POST',table,'createModal')" class="px-4 py-2 text-sm font-medium text-white bg-blue-600 rounded-lg hover:bg-blue-700">Save Client</button>
        </div>
    </div>
</div>
...
<div id="viewModal" class="hidden fixed inset-0 z-50 flex items-center justify-center modal-overlay" data-modal-id="viewModal">
    <div class="bg-white rounded-xl shadow-2xl w-full max-w-lg mx-4">
        <div class="flex items-center justify-between px-6 py-4 border-b border-gray-100">
            <h3 class="text-lg font-bold text-gray-900">Client Details</h3>
...
function viewClient(id){
    $.get('{{ url('/clients') }}/'+id, function(d){
        document.getElementById('viewContent').innerHTML=
            ...
            '<div class="grid grid-cols-3 gap-4"><div><p class="text-xs text-gray-500 mb-1">City</p><p class="text-sm">'+(d.city||'—')+'</p></div><div><p class="text-xs text-gray-500 mb-1">State</p><p class="text-sm">'+(d.state||'—')+'</p></div><div><p class="text-xs text-gray-500 mb-1">ZIP Code</p><p class="text-sm">'+(d.zip||'—')+'</p></div></div>'+
            '</div>';
        openModal('viewModal');
    });
}

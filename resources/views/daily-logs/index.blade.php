...
$(document).ready(function() {
            ...
            { data: 'actions', name: 'actions', orderable: false, searchable: false, render: function(data, type, row) {
                return `
                    <div class="flex gap-1 items-center justify-center">
                        <button type="button" onclick="window.location='{{ route('projects.daily-logs.show', [$project, '']) }}'.replace('__ID__', ${data})" class="inline-flex items-center justify-center w-8 h-8 rounded-lg text-blue-600 hover:bg-blue-50 hover:text-blue-700 transition" title="View">
                            <svg class="w-5 h-5 flex-shrink-0" fill="none" stroke="currentColor" stroke-width="1.5" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" d="M2.036 12.322a1.012 1.012 0 010-.639C3.423 7.51 7.36 4.5 12 4.5c4.638 0 8.573 3.007 9.963 7.178.07.207.07.431 0 .639C20.577 16.49 16.64 19.5 12 19.5c-4.638 0-8.573-3.007-9.963-7.178z"/><path stroke-linecap="round" stroke-linejoin="round" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z"/></svg>
                        </button>
                        <button type="button" onclick="loadEdit('{{ route('projects.daily-logs.edit', [$project, '']) }}'.replace('__ID__', ${data}), 'editForm', 'editModal', ['date', 'weather', 'temperature', 'notes', 'visitors', 'safety_issues', 'delays'])" class="inline-flex items-center justify-center w-8 h-8 rounded-lg text-amber-600 hover:bg-amber-50 hover:text-amber-700 transition" title="Edit">
                            <svg class="w-5 h-5 flex-shrink-0" fill="none" stroke="currentColor" stroke-width="1.5" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" d="M16.862 4.487l1.687-1.688a1.875 1.875 0 112.652 2.652L10.582 16.07a4.5 4.5 0 01-1.897 1.13L6 18l.8-2.685a4.5 4.5 0 011.13-1.897l8.932-8.931zm0 0L19.5 7.125M18 14v4.75A2.25 2.25 0 0115.75 21H5.25A2.25 2.25 0 013 18.75V8.25A2.25 2.25 0 015.25 6H10"/></svg>
                        </button>
                        <button type="button" onclick="confirmDelete('{{ route('projects.daily-logs.destroy', [$project, '']) }}'.replace('__ID__', ${data}), 'dailyLogsTable')" class="inline-flex items-center justify-center w-8 h-8 rounded-lg text-red-600 hover:bg-red-50 hover:text-red-700 transition" title="Delete">
    ...
    // Store the edit URL base for use in loadEdit
    window.editUrlBase = '{{ route('projects.daily-logs.update', [$project, '']) }}';
});

// Override the loadEdit function to handle edit URL properly
var originalLoadEdit = loadEdit;
function loadEdit(url, formId, modalId, fields) {
    // Extract the ID from the URL
    var parts = url.split('/');
    var id = parts[parts.length - 1];

    // Update the edit form action
    document.getElementById(formId).action = window.editUrlBase.replace('__ID__', id);

    // Call the original function
    originalLoadEdit(url, formId, modalId, fields);
}
</script>
@endpush
@endsection

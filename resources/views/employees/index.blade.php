...
<div class="flex items-center justify-between mb-6">
    ...
    <div class="flex items-center gap-2">
        <a href="{{ route('exports.employees') }}" class="inline-flex items-center gap-2 bg-white hover:bg-emerald-50 text-emerald-700 text-sm font-semibold px-4 py-2.5 rounded-lg shadow-sm border border-emerald-200 transition" title="Download all employees as Excel">
...
{{-- Status tabs — keeps terminated/inactive employees out of the default
     Active roster the office works with day-to-day. Server-side filter via
     DataTables `status` param wired in the ajax callback below. --}}
<div class="mb-4 flex items-center gap-1 border-b border-gray-200" id="employeeStatusTabs">
    @php
        $tabs = [
            'active'     => ['label' => 'Active',     'pill' => 'bg-green-100 text-green-700'],
            'inactive'   => ['label' => 'Inactive',   'pill' => 'bg-gray-100 text-gray-700'],
            'terminated' => ['label' => 'Terminated', 'pill' => 'bg-red-100 text-red-700'],
            ''           => ['label' => 'All',        'pill' => 'bg-blue-100 text-blue-700'],
        ];
    @endphp
    @foreach ($tabs as $key => $t)
        <button type="button" data-status="{{ $key }}"
            class="tab-btn px-4 py-2 text-sm font-medium border-b-2 -mb-px transition
                   {{ $key === 'active' ? 'border-blue-600 text-blue-700' : 'border-transparent text-gray-500 hover:text-gray-700' }}">
            {{ $t['label'] }}
        </button>
    @endforeach
</div>
...
@push('scripts')
<script>
// Default-open tab: Active (matches the highlighted tab above).
var employeeStatusFilter = 'active';

var table = $('#dataTable').DataTable({
    ajax: {
        url: '{{ route("employees.index") }}',
        data: function(d) { d.status = employeeStatusFilter; },
    },
    columns: [
...
// Tab click: swap the filter value, reload the DataTable, update highlight.
document.getElementById('employeeStatusTabs').addEventListener('click', function(e) {
    var btn = e.target.closest('.tab-btn');
    if (!btn) return;
    employeeStatusFilter = btn.dataset.status;
    document.querySelectorAll('#employeeStatusTabs .tab-btn').forEach(function(b) {
        if (b === btn) {

    ...
    <div class="relative top-20 mx-auto p-5 border w-full max-w-md shadow-lg rounded-md bg-white">
        ...
        <form id="createForm" method="POST" action="{{ route('projects.commitments.store', $project) }}">
            ...
            <div class="flex gap-3 justify-end">
                ...
                <button type="button" onclick="submitForm('createForm', '{{ route('projects.commitments.store', $project) }}', 'POST', commitmentTableInstance, 'createModal')" class="px-4 py-2 text-sm font-medium text-white bg-blue-600 rounded-md hover:bg-blue-700">
                    Create
                </button>
            </div>
        </form>
...
var commitmentTableInstance = null;
let vendorsCache = [];
let costCodesCache = [];
...
function renderActions(id) {
    return `
        <div class="flex items-center justify-center gap-2">
            ...
            <button type="button" onclick="confirmDelete('/projects/{{ $project->id }}/commitments/${id}', commitmentTableInstance)" class="inline-flex items-center justify-center w-8 h-8 rounded-lg text-red-600 hover:bg-red-50 hover:text-red-700 transition" title="Delete">
                <svg class="w-5 h-5 flex-shrink-0" fill="none" stroke="currentColor" stroke-width="1.5" viewBox="0 0 24 24">
...
document.addEventListener('DOMContentLoaded', function() {
    document.getElementById('editSubmitBtn').addEventListener('click', function() {
        ...
        submitForm('editForm', url, 'PUT', commitmentTableInstance, 'editModal');
    });

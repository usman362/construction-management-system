    ...
    <div class="bg-white rounded-lg shadow p-8 max-w-2xl mx-auto">
        ...
        <form method="POST" action="{{ route('invoices.store') }}">
            ...
            <div class="mb-4">
                ...
                <select name="commitment_id" id="commitment_id" class="w-full border-gray-300 rounded-lg shadow-sm">
                    ...
                    @foreach ($commitments as $commitment)
                        <option value="{{ $commitment->id }}" {{ old('commitment_id') == $commitment->id ? 'selected' : '' }}>
            ...
            <div class="mb-4">
                ...
                <input type="text" name="invoice_number" id="invoice_number" required value="{{ old('invoice_number') }}" class="w-full border-gray-300 rounded-lg shadow-sm @error('invoice_number') border-red-500 @enderror">
            ...
            <div class="mb-4">
                ...
                <input type="date" name="invoice_date" id="invoice_date" required value="{{ old('invoice_date') }}" class="w-full border-gray-300 rounded-lg shadow-sm @error('invoice_date') border-red-500 @enderror">
                @error('invoice_date')
                    <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div class="mb-6">
                ...
                <input type="date" name="due_date" id="due_date" required value="{{ old('due_date') }}" class="w-full border-gray-300 rounded-lg shadow-sm @error('due_date') border-red-500 @enderror">
                @error('due_date')
                    <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                @enderror
            </div>
